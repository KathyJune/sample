<template>
  <div class="date-content">
    <div class="date-header"
         v-if="type != 'time'">
      <span class="date-year-left-picker"
            @click.stop="updateView('default', -1)"><i class="fa fa-angle-left"></i><i class="fa fa-angle-left"></i></span>
      <span class="date-month-left-picker"
            @click.stop="updateView('month', -1)"
            v-show="view=='date'||view=='week'"><i class="fa fa-angle-left"></i></span>
      <span class="date-header-show"
            @click.stop="changeView('year')"
            v-if="view != 'year'">{{nowView.year()}}{{$t('date.header.year')}}</span>
      <span class="date-header-show"
            v-if="view == 'year'">{{nowView.year()-6}}&nbsp;&nbsp;-&nbsp;&nbsp;{{nowView.year()+5}}{{$t('date.header.year')}}</span>
      <span class="date-header-show"
      ...
      <span class="date-header-show"
            ...
            v-show="view == 'hour' || view == 'minute'">{{nowView.date()}}{{$t('date.header.day')}}</span>
      <span class="date-year-right-picker"
            @click.stop="updateView('default', 1)"><i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i></span>
      <span class="date-month-right-picker"
            @click.stop="updateView('month', 1)"
            v-show="view=='date'||view=='week'"><i class="fa fa-angle-right"></i></span>
    </div>
    <div class="date-header"
         v-show="view=='minute'">
      <span class="date-month-left-picker"
            @click.stop="updateView('hour', -1)"><i class="fa fa-angle-left"></i></span>
      <span class="date-header-show"
            @click.stop="changeView('hour')">{{nowView | hoursString}}</span>
      <span class="date-month-right-picker"
            @click.stop="updateView('hour', 1)"><i class="fa fa-angle-right"></i></span>
    </div>
    <div :class="dateBodyCls">
      <div class="date-body-weeks"
           v-if="view=='date'"><span v-for="w of weeks" :key="w">{{w}}</span></div>
      <div class="date-body-pickers">
        <span v-for="d of dates" :key="d.string"
              :string="d.string"
              :class="{'date-not-now-day': !d.isNowDays, 'date-today':d.isToday, 'date-selected': isSelected(d), 'date-range-selected': isRangeSelected(d), 'date-disabled': d.disabled}"
...
export default {
  ...
  props: {
    ...
    startWeek: {
      type: Number,
      default: () => config.startWeek
    }
  },
  data () {
    return {
      options: utils.merge({}, options.datetimeOptions, this.option),
      today: manba(),
  ...
  watch: {
    type () {
      this.options = utils.merge({}, options.datetimeOptions, this.option)
  ...
  computed: {
    ...
    weeks () {
      let weeks = [
        this.$t('date.weeks.monday'),
        this.$t('date.weeks.tuesday'),
        this.$t('date.weeks.wednesday'),
        this.$t('date.weeks.thursday'),
        this.$t('date.weeks.friday'),
        this.$t('date.weeks.saturday'),
        this.$t('date.weeks.sunday')
      ]
