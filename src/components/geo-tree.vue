...
export default {
  ...
  created () {
    ...
    this.$on('tree-node-drag-over', (event, treeNode) => {
      ...
      const draggingNode = dragState.draggingNode || parent.draggingNode
      ...
      if (draggingNode.node && (draggingNode.node === dropNode.node || draggingNode.node.contains(dropNode.node))) {
        dropPrev = false
        dropInner = false
        dropNext = false
      }
      ...
      } else if (draggingNode.status === 'new') {
        this.$emit('tree-node-drag-add', event, dropNode.node, draggingNode, dropType)
      }
    })
    ...
    this.$on('tree-node-drag-add', (event, treeNode, draggingNode, dropType) => {
      const data = draggingNode.data
      treeNode.insertChild({ data })
      draggingNode.status = 'drop'
      draggingNode = null
      this.$emit('tree-drag-add-end', event)
    })
  },
